...
оба пакета связки — `msordertranslator` (передатчик, он же провайдер эндпоинтов)
и `msorderreceiver` (приёмник), — то есть сборка и smoke провайдера идут на одной
...
| Режим | Что видно |
|---|---|
| `all` (по умолчанию) | весь публичный контракт сайта — каталог работает как документация: интегратор видит, какие scope существуют |
...
Данные при этом недоступны — вызов даёт `insufficient_scope` или
`insufficient_permission`, — но поверхность записи раскрывается. Активный режим
возвращается в `meta.filter`, чтобы короткий каталог не выглядел как «эндпоинтов
нет».
